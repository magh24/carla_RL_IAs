...
def create_resnet_basic_block(width_output_feature_map, height_output_feature_map, nb_channel_in, nb_channel_out):
    basic_block = nn.Sequential(
        ...
        nn.Conv2d(nb_channel_in, nb_channel_out, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False),
        nn.BatchNorm2d(nb_channel_out, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True),
        nn.ReLU(inplace=True),
        nn.Conv2d(nb_channel_out, nb_channel_out, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False),
        nn.BatchNorm2d(nb_channel_out, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True),
    )
    return basic_block
...
class Model_Segmentation_Traffic_Light_Supervised(nn.Module):
    def __init__(self, nb_images_input, nb_images_output, hidden_size, nb_class_segmentation, nb_class_dist_to_tl, crop_sky = False):
        ...
        self.fc2_tl_inters_none = nn.Linear(hidden_size,
                                            4)  # Classification traffic_light US, traffic_light EU, intersection, none
        self.fc2_traffic_light_state = nn.Linear(hidden_size,
                                                 2)  # Classification red/orange or green (maybe 3 classes in stead of 2?)
        self.fc2_distance_to_tl = nn.Linear(hidden_size,
                                            nb_class_dist_to_tl)  # classification on the distance to traffic_light

        self.fc1_delta_y_yaw_camera = nn.Linear(self.size_state_RL, int(
            hidden_size / 4))  # Hard coded here, we want a little hidden size...
        self.fc2_delta_y_yaw_camera = nn.Linear(int(hidden_size / 4),
                                                2 * nb_images_output)  # Regression on delta_y and delta_yaw of each input frame!
        ...
        self.last_conv_segmentation = nn.Conv2d(32, nb_class_segmentation * nb_images_output, kernel_size=(1, 1),
                                                stride=(1, 1), bias=False)
        self.last_bn = nn.BatchNorm2d(nb_class_segmentation * nb_images_output, eps=1e-05, momentum=0.1, affine=True,
